...
import { fullYearPeriod, getPeriodLabel, months } from "../utils/monthPeriod.js";
import Button from "./Button.jsx";

export default function PeriodFilter({ period = fullYearPeriod, onApply = () => {}, disabled = false }) {
  ...
  return (
    <section className="relative mb-5">
      <div className="flex flex-wrap items-center gap-2">
        <Button variant={isFiltered ? "primary" : "secondary"} onClick={openFilter} aria-expanded={isOpen} disabled={disabled}>
          <span className="flex items-center gap-2">
            <CalendarRange className="h-4 w-4" strokeWidth={2.2} />
            Filtrar visualizacao: {getPeriodLabel(period)}
      ...
      {isOpen && (
        <div className="app-panel absolute left-0 top-full z-40 mt-2 grid w-full max-w-lg gap-4 p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
          <label className="grid gap-1.5">
            <span className="form-label">Mes inicial</span>
          ...
          <label className="grid gap-1.5">
            <span className="form-label">Mes final</span>
